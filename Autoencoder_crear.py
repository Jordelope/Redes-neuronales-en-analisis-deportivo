...
archivo_encod = r"" 
archivo_decod = r"" 
archivo_autoencoder = r"redes_disponibles\finales\overcomplete\AE_over_shtgStatPG.json" 
...
descripcion = " Autoencoder overcomplete para tratar estadisticas de tiro en per game."

## ESTRUCTURA autoencoder (si no tenemos los MLP) ##

input_sz = 30            # Número de entradas
lat_spc_dim = 80         # Dimension espacio latente(salida encoder, entrada decoder) 

estructura_encod = [64]               # Capas ocultas encoder
estructura_decod = estructura_encod[::-1]      # Capas ocultas decoder

lista_act_encod = [F.relu, torch.tanh]  # Funciones activacion encoder (None = [None,...,None] por defecto lineal en MLP)
lista_act_decod = [ F.leaky_relu] + [None] # Funciones activacion encoder (None = [None,...,None] por defecto lineal en MLP)
